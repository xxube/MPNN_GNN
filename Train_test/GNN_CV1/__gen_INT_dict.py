...
def main():
    ...
    model = _optuna.create_model(trial_setup_dict).to(torch.device('cpu'))
    optimizer = _optuna.create_optimizer(trial_setup_dict, model)
    model, optimizer = restore_checkpoint(model, optimizer, model_dir)
